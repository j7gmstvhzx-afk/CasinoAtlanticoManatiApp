  ...
  return (
    ...
    >
      ...
      <View style={styles.left}>
        <Text variant="bodyStrong" style={{ fontWeight: '700', color: NAVY }}>{machine.id}</Text>
      ...
      <View style={styles.center}>
        <Text variant="small" style={{ fontWeight: '600', color: NAVY }} numberOfLines={1}>{machine.game}</Text>
        <Text variant="caption" tone="muted" numberOfLines={1}>{machine.manufacturer}</Text>
        <View style={styles.badges}>
          <View style={[styles.typeBadge, { borderColor: typeColor + '55', backgroundColor: typeColor + '12' }]}>
            <Text variant="caption" style={{ color: typeColor, fontSize: 10 }}>{machine.type}</Text>
          </View>
          <Text variant="caption" tone="muted">
            {machine.denomination === '01/02/05/10' ? 'Multi' : machine.denomination.replace('0.', '') + '¢'}
          </Text>
        </View>
      </View>
